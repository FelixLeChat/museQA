...
class with_engine(object):
    ...
    :param engine_name: 'main' or the name of one of the subordinates, if is ``None``
             it will not force any specific engine.
    :param main_params: A dictionary or GET parameters that when present will force
             usage of the main node. The keys of the dictionary will be the
             ...
             If `main_params` is a list then it is converted to a dictionary where
             the keys are the entries of the list and the value is always True.
    """

    def __init__(self, engine_name=None, main_params={}):
        self.engine_name = engine_name

        if not hasattr(main_params, 'keys'):
            self.main_params = dict((p, True) for p in main_params)
        else:
            self.main_params = main_params

    def before_validate(self, remainder, params):
        force_request_engine(self.engine_name)
        for p, pop in self.main_params.items():
            if p in params:
                if pop:
                    v = params.pop(p, None)
                else:
