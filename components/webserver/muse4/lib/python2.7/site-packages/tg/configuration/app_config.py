...
class AppConfig(Bunch):
    ...
    def setup_sqlalchemy(self):
        ...
        balanced_main = config.get('sqlalchemy.main.url')
        if not balanced_main:
            engine = engine_from_config(config, 'sqlalchemy.')
        else:
            engine = engine_from_config(config, 'sqlalchemy.main.')
            config['balanced_engines'] = {'main':engine,
                                          'subordinates':{},
                                          'all':{'main':engine}}

            all_engines = config['balanced_engines']['all']
            subordinates = config['balanced_engines']['subordinates']
            for entry in config.keys():
                if entry.startswith('sqlalchemy.subordinates.'):
                    ...
                    all_engines[subordinate_name] = subordinates[subordinate_name] = engine_from_config(config, subordinate_config+'.')

            if not config['balanced_engines']['subordinates']:
