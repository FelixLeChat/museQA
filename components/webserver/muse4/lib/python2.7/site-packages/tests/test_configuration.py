...
class TestAppConfig:
    ...
    def test_setup_sqla_balanced(self):
        config['sqlalchemy.main.url'] = 'sqlite://'
        config['sqlalchemy.subordinates.subordinate1.url'] = 'sqlite://'
        self.config.use_sqlalchemy = True
        ...
        self.config.use_sqlalchemy = False
        config.pop('sqlalchemy.main.url')
        config.pop('sqlalchemy.subordinates.subordinate1.url')

    @raises(TGConfigError)
    def test_setup_sqla_balanced_prevent_subordinate_named_main(self):
        config['sqlalchemy.main.url'] = 'sqlite://'
        config['sqlalchemy.subordinates.main.url'] = 'sqlite://'
        self.config.use_sqlalchemy = True
        ...
        try:
            ...
            self.config.use_sqlalchemy = False
            config.pop('sqlalchemy.main.url')
            config.pop('sqlalchemy.subordinates.main.url')

    @raises(TGConfigError)
    def test_setup_sqla_balanced_no_subordinates(self):
        config['sqlalchemy.main.url'] = 'sqlite://'
        self.config.use_sqlalchemy = True
