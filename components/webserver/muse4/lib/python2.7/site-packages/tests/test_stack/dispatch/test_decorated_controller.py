...
class TestDecorators(object):
    ...
    def test_with_engine(self):
        resp = self.app.get('/onmain')
        assert 'mainsubordinate' in resp

    def test_with_engine_nopop(self):
        resp = self.app.get('/onmain?second=1')
        assert 'main' in resp
        assert 'second' in resp

    def test_with_engine_pop(self):
        resp = self.app.get('/onmain?first=1')
        assert 'main' in resp
        assert 'first' not in resp

    def test_with_engine_using_list(self):
        resp = self.app.get('/onmain_withlist?first=1')
        assert 'main' in resp
        assert 'first' not in resp
